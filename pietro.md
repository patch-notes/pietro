...
6. **M6 — React UI (2-3 days).** ✅ *Shipped 2026-05-14.* Three pages (`/`, `/new`, `/login`) per §14, Tailwind v4 via the official `@tailwindcss/vite` plugin (one CSS file: `@import "tailwindcss";`), react-router v7, a 110-line typed fetch wrapper (`src/api.ts`) with an `ApiError` class that parses the project's `{ error: { code, message } }` envelope. `RequireAuth` guard in `App.tsx` does the 401 → `/login` redirect via a three-state session probe (`loading | out | in`) so there's no flash of the login screen on initial load. Dashboard surfaces all keys (the backend's list query has no `revoked_at IS NULL` filter, so revoked rows are honestly rendered, dim'd, with a badge). Mint flow is two-phase on one component: form, then a prominent amber banner + monospace plaintext block + Copy button (with a `document.createRange` fallback for insecure contexts) + explicit "I've saved it" link. No auto-redirect on success — the user must acknowledge the plaintext-once contract. Bundle: 243 KB JS / 77 KB gzipped (under the §14.2 200 KB-gz target). One new runtime dep: `react-router-dom`. Backend untouched; 55/55 tests still green.
7. **M7 — Embed + release (½ day).** ✅ *Shipped 2026-05-15.* `rust-embed = "8"` (one new crate, prod count 24/25). New `src/spa.rs` provides two handlers: `serve_asset` for `/assets/{*path}` (real-miss → JSON-404; no SPA fallback under `/assets/` because Vite content-addresses every file) and `fallback` mounted on `Router::fallback` (SPA `index.html` for any unknown non-API path; JSON-404 envelope under `/api/*` and `/proxy/*` per §7). A 10-line content-type table covers JS/CSS/HTML/SVG/PNG/ICO/JSON/woff(2)/map/txt — no `mime_guess` dep needed. Hard debug-vs-release split: in `#[cfg(debug_assertions)]` builds both handlers return a hostname-agnostic dev-mode notice page (the §13 "developer who hits :8080 directly gets a clear note, not a 4-day-old SPA"); in release they serve the embedded bytes. `build.rs` re-runs only when `frontend/dist/index.html` or `frontend/dist/assets/` changes; it **fails the release build** when `index.html` is missing (with a clear "run `cd frontend && npm run build`" hint) but only **warns** in debug so backend-only dev work isn't blocked. `.github/workflows/release.yml` builds tagged tarballs + sha256 on tag push (`v*`) for `x86_64-` and `aarch64-unknown-linux-musl` (x86_64 via host `musl-tools`, aarch64 via `cross`), uploading via `softprops/action-gh-release@v2`. Tests: **60/60** — 55 baseline plus 3 new router tests (`spa_fallback_serves_html_for_unknown_non_api_path`, `unknown_api_path_returns_json_404`, `unknown_proxy_path_returns_json_404`) and 2 new `spa::tests` unit tests (content-type table, API/proxy path classifier). Release binary: **11 MB stripped**, well under the §13 15–25 MB target. Live release smoke proved: `/` serves the real embedded `index.html`, `/assets/<hash>.js` serves 243 KB of `application/javascript`, `/some/spa/route` serves the same `index.html` bytes (history-mode), `/api/garbage` and `/assets/missing` JSON-404 through the project envelope.
...
- **2026-05-14 — M6 shipped.** Wired `@tailwindcss/vite` + a Vite dev-server proxy (`/api`, `/proxy` → `http://127.0.0.1:18080`) in `vite.config.ts`. Replaced the Vite splash template with a router-driven SPA: `src/App.tsx` (router + `RequireAuth` + three-state session probe), `src/api.ts` (typed fetch wrapper + `ApiError`), and three pages under `src/pages/`: `Login.tsx` (single `<a href="/api/auth/login">` so the 303 OIDC chain isn't broken by `fetch`), `Dashboard.tsx` (key list with optimistic-revoke + service badge + dim'd revoked rows + sign-out header), `NewKey.tsx` (form → plaintext-once reveal with amber banner, copy button + selection fallback, and an explicit "I've saved it" link instead of auto-redirect). One new runtime dep: `react-router-dom` ^7. The bundle is **243 KB JS / 77 KB gzipped** — well under the §14.2 200 KB-gz target. Lint clean (`eslint`, `tsc --noEmit`), build clean (`vite build`), and the live end-to-end smoke confirmed: SPA served by Vite at `:5174`, all five session-guarded API endpoints return 401-with-JSON-envelope through the proxy, `/api/auth/login` correctly 303s to the IdP through the proxy with full PKCE+state+nonce in the URL. Also tidied up: removed orphaned Vite template assets (`hero.png`, `react.svg`, `vite.svg`, `icons.svg`), renamed `<title>` to "Pietro", deleted the orphaned `App.css`. Backend untouched; 55/55 Rust tests still green. Pre-step cleanup commit (`e72bb5a`) cleared seven `cargo clippy -D warnings` lints and one rustfmt drift that had snuck past M5 — `cargo fmt`, `cargo clippy -D warnings`, and `cargo test` are now all clean and gating every future step.
- **2026-05-15 — M7 shipped.** Added `rust-embed = "8"` (one new crate, takes prod count to 24/25 exactly). `src/spa.rs` (~180 LOC including tests) is the entire embed surface: two handlers (`serve_asset`, `fallback`), a 10-line content-type table covering the bundle shape (no `mime_guess` dep), and a hard `#[cfg(debug_assertions)]` split so debug builds never accidentally serve stale embedded bytes — they return a hostname-agnostic dev-mode notice page instead. `build.rs` re-runs only on changes to `frontend/dist/index.html` or `frontend/dist/assets/`; it fails the build in release mode when `index.html` is missing (with a clear hint to `cd frontend && npm run build`), but only warns in debug so backend-only dev work isn't blocked. `Router::fallback` and the `/assets/{*path}` route are wired in `routes.rs`. The §7 carve-out is preserved by `is_api_or_proxy_path`: anything under `/api/` or `/proxy/` that didn't match a real handler returns the project JSON-404 envelope rather than the SPA. Tests: **60/60** — 3 new router tests (HTML for SPA paths, JSON-404 for `/api/garbage`, JSON-404 for `/proxy/`) and 2 new unit tests (content-type table, path classifier). Release binary measured **11 MB stripped** (under the §13 15–25 MB target), and a live release-mode smoke run proved end-to-end: `/` → embedded `index.html`, `/assets/<hash>.js` → 243 KB `application/javascript`, `/some/spa/route` → identical `index.html` bytes (history-mode), `/api/garbage` and `/assets/missing` → JSON-404. `.github/workflows/release.yml` builds `pietro-<tag>-{x86_64,aarch64}-unknown-linux-musl.tar.gz` + `.sha256` on tag push using host `musl-tools` for x86_64 and `cross` for aarch64, uploading via `softprops/action-gh-release@v2`. Pietro v1 is complete.
