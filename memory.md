...
Updated: 2026-05-14 (M2 complete)
...
- **Crate budget in `Cargo.toml`** (after M2):
  - prod: axum 0.8, tokio 1, clap 4, serde 1, serde_yaml 0.9, url 2, regex 1, anyhow 1, thiserror 2, tracing 0.1, tracing-subscriber 0.3, **sqlx 0.8** (features: runtime-tokio, sqlite, macros, migrate; `default-features = false`, no TLS).
  - dev: tower 0.5 (for `ServiceExt::oneshot`).
  - Remaining for later milestones: reqwest (M5), openidconnect + axum-extra (M3), rust-embed (M7), blake3 + rand (M4), tower-http (TBD).
...
- **Dead-code attributes**: module-level `#![allow(dead_code)]` is used in files whose fields are validated now but consumed later (currently `config.rs`, `errors.rs`, `secret.rs`). Plus field-level `#[allow(... reason = "...")]` for individual items (e.g. `AppState::config`). As each milestone wires them up, the attribute is removed. Never use a crate-wide `allow`.
- **Smoke test recipe**:
  ```
  rm -f pietro.db*
  ...
  cargo run -- migrate --config pietro.yaml   # creates pietro.db with full schema
  cargo run -- serve   --config pietro.yaml   # listens on :18080
